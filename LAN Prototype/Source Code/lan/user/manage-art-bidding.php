...
if (isset($_GET['pageno'])) {
        ...
        $total_pages_sql = "SELECT COUNT(*) FROM art a INNER JOIN artist ar ON a.ArtistID = ar.ID INNER JOIN art_bidding ab ON a.ID = ab.ArtID INNER JOIN user u ON a.UserID = u.ID WHERE u.ID='$userid'";
$ret1=mysqli_query($con,"SELECT COUNT(*) FROM art a INNER JOIN artist ar ON a.ArtistID = ar.ID INNER JOIN art_bidding ab ON a.ID = ab.ArtID INNER JOIN user u ON a.UserID = u.ID WHERE u.ID='$userid'");
$total_rows = mysqli_fetch_array($ret1)[0];
$total_pages = ceil($total_rows / $no_of_records_per_page);
 $query=mysqli_query($con,"SELECT a.ID as ArtID,a.Name as ArtName,a.Price as Price,Category,a.Review as Review,ab.StartDate,ab.EndDate,a.CreationDate,a.LastUpdate, ar.Name as ArtistName,CONCAT(FirstName, ' ', LastName) AS UserName,Image FROM art a INNER JOIN artist ar ON a.ArtistID = ar.ID INNER JOIN art_bidding ab ON a.ID = ab.ArtID INNER JOIN user u ON a.UserID = u.ID WHERE u.ID='$userid' ORDER BY a.ID LIMIT $offset, $no_of_records_per_page");
